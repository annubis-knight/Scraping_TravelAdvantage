...
function createMarkers(data) {
    ...
    data.forEach(ville => {
        const filteredPercentages = ville.pourcentages.filter((percentage, index) => 
            ...
            marker.bindPopup(() => {
                ...
                ['Weekend', 'Semaine', '2 Semaines'].forEach((type, index) => {
                    if ((index === 0 && filters.weekend) || 
                        ...
                        checkbox.setAttribute('data-percentage', ville.pourcentages[index]);
                        checkbox.checked = selectedCities[currentMonth] && 
                                           selectedCities[currentMonth][ville.ville] && 
                                           selectedCities[currentMonth][ville.ville].types.some(t => t.type === type);
...
function updateFilters() {
    ...
    handlePercentageFilter();
}

function handlePercentageFilter() {
    const minInput = document.getElementById('min-percentage');
    const maxInput = document.getElementById('max-percentage');
    
    minPercentage = Math.max(0, Math.min(100, parseInt(minInput.value) || 0));
    maxPercentage = Math.max(0, Math.min(100, parseInt(maxInput.value) || 100));
...
function handleCheckboxClick(e) {
    ...
    if (checkbox.checked) {
        ...
        selectedCities[currentMonth][ville].types.push({ type: type, pourcentage: percentage });
    } else {
        if (selectedCities[currentMonth][ville]) {
            selectedCities[currentMonth][ville].types = selectedCities[currentMonth][ville].types.filter(t => t.type !== type);
...
function updateSelectedCitiesList() {
    ...
    if (selectedCities[currentMonth]) {
        Object.entries(selectedCities[currentMonth]).forEach(([ville, data]) => {
            data.types.forEach(typeData => {
                ...
                li.textContent = `${ville} (${data.country}): ${typeData.type} (${typeData.pourcentage}%)`;
                ...
                removeButton.onclick = (e) => {
                    e.stopPropagation();
                    removeSelectedCity(ville, typeData.type);
                };
    ...
    // Mettre à jour la visibilité du bouton "Créer Template"
...
function handleCreateTemplate() {
    console.log("Données envoyées au serveur:", JSON.stringify(selectedCities, null, 2));
    fetch('/saveSelectedCities', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            window.open('/templateBrevo', '_blank');
        } else {
...
// Ajouter des écouteurs d'événements pour les champs de saisie de pourcentage
document.getElementById('min-percentage').addEventListener('input', debounce(handlePercentageFilter, 300));
document.getElementById('max-percentage').addEventListener('input', debounce(handlePercentageFilter, 300));
